...
Ollama required: https://ollama.com/   then: ollama pull huihui_ai/qwen3.5-abliterated:9b
               ollama create metatron-qwen -f Modelfile  (if custom)
"""
...
os.makedirs(os.path.expanduser("~/METATRON/reports"), exist_ok=True)
...
def export_pdf(data: dict, output_dir: str) -> str:
    ...
    # output_dir already exists (created at top of file), but keep exist_ok for safety
...
<section><h2>Fixes &amp; Mitigations</h2>{'</table><thead><tr><th>#</th><th>Vuln</th><th>Fix</th><th>Source</th></tr></thead><tbody>' + fix_rows + '</tbody></td>' if data["fixes"] else '<p>None recorded.</p>'}</section>
<section><h2>Exploits Attempted</h2>{'<tr><thead><tr><th>#</th><th>Exploit</th><th>Tool</th><th>Payload</th><th>Result</th></tr></thead><tbody>' + exp_rows + '</tbody></table>' if data["exploits"] else '<p>None recorded.</p>'}</section>
...
if __name__ == "__main__":
    ...
    init_db()
    # quick check for Ollama with helpful messages
    try:
        requests.get("http://localhost:11434", timeout=2)
        print("[+] Ollama detected.")
    except:
        warn("Ollama not reachable. Make sure it's running and the model is created.")
        print("   Start Ollama:   ollama serve")
        print("   Pull base model: ollama pull huihui_ai/qwen3.5-abliterated:9b")
        print("   Create custom:   ollama create metatron-qwen -f Modelfile (optional)")
    main_menu()
